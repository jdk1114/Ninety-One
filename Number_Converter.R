...
reversedInput <- paste(rev(strsplit(input, "")[[1]]), collapse = "")
digitEndLocations <- sort(nchar(input) - gregexpr("([0-9]+)", reversedInput)[[1]] + 1)
if (digitStartLocations[1] != 1) {
  if (strsplit(input, "")[[1]][digitStartLocations[1] - 1] != " ") {
    output <- "number invalid"
  }
}
if (digitEndLocations[1] != nchar(input)) {
  if (strsplit(input, "")[[1]][digitEndLocations[1] + 1] != " ") {
    output <- "number invalid"
  }
}
