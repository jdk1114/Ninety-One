...
input <- read.delim("C:/Users/justi/OneDrive/Documents/Job applications/UK CV/91/Examples.txt", header = FALSE, stringsAsFactors = FALSE)

## Manually entering an input
if (is.null(input)) {
  input <- "The pump is 23 456,9 KGs deep underground"  
}
...
output <- sapply(1:nrow(input), function(i) {
  ...
  reversedInput <- paste(rev(strsplit(input[i,], "")[[1]]), collapse = "")
  digitEndLocations <- sort(nchar(input[i,]) - gregexpr("([0-9]+)", reversedInput)[[1]] + 1)
  if (digitStartLocations[1] != 1) {
    if (strsplit(input[i,], "")[[1]][digitStartLocations[1] - 1] != " ") {
      output <- "number invalid"
    }
  }
  if (digitEndLocations[1] != nchar(input[i,])) {
    if (strsplit(input[i,], "")[[1]][digitEndLocations[1] + 1] != " ") {
      output <- "number invalid"
    }
  }
  ...
  if (is.null(output)) {
    ## Separating the digits into a vector
    ...
    doubleDigitFn <- function(digits) {
      ...
      ifelse(as.numeric(paste(digits, collapse = "")) %in% doubleDigits,
             ...
             paste(names(tens)[tens == as.numeric(paste0(digits[1], 0))],
                   ifelse(digits[2] != "0", names(singleDigits)[singleDigits == as.numeric(digits[2])], ""),
    ...
    hundredsFn <- function(digits) {
      ...
      paste0(ifelse(!(digits[1] %in% "0"),
                    paste(names(singleDigits)[singleDigits == as.numeric(digits[1])], "hundred"),
                    ""
      ), 
      paste0(ifelse(!(digits[1] %in% "0") & lastDigits != "", " and ", ""),ifelse(lastDigits != "", lastDigits, ""))
      )
    }
    ...
    magnitudesFn <- function(digits) {
      ...
      if (!(all(digits[1:prefixMagnitude] %in% "0"))) {
        ...
        output <- paste(switch(as.character(prefixMagnitude),
                               "1" = names(singleDigits)[singleDigits == as.numeric(digits[1])], 
    ...
    if (numberOfDigits == 1) {
      output <- names(singleDigits)[singleDigits == as.numeric(input)]
